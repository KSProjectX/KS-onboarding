...
async def setup_programme(request: SetupRequest):
    ...
    try:
        ...
        # Only update tech_stack if not already set and message contains specific tech mentions
        if not setup_data.get("tech_stack") and any(tech in message_lower for tech in specific_tech):
        ...
        # Only update timeline if not already set and message contains specific timeline/budget info
        if not setup_data.get("timeline") and (any(keyword in message_lower for keyword in timeline_keywords) or any(indicator in message_lower for indicator in budget_indicators)):
        ...
        completeness = 0
        if setup_data.get("industry"): completeness += 25
        if setup_data.get("problem_statement"): completeness += 25
        if setup_data.get("tech_stack"): completeness += 25
        if setup_data.get("timeline"): completeness += 25
        ...
        missing_info = []
        if not setup_data.get("industry") or not setup_data.get("problem_statement"):
